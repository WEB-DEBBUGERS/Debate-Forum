...
export default function Comments({ postId, userData }) {
  ...
  useEffect(() => {
    ...
    const unsubscribeComments = onValue(commentsQuery, (snapshot) => {
      const fetchedComments = snapshot.exists()
    ...
    const unsubscribeReplies = onValue(repliesQuery, (snapshot) => {
      const fetchedReplies = snapshot.exists()
        ? Object.values(snapshot.val())
        : [];
  ...
  return (
    <div>
      <form onSubmit={handleCommentSubmit}>
        ...
        />
        <button type="submit" style={{ color: "black", fontSize: "15px" }}>
          💬
        </button>
      </form>
      ...
      <div className="comments">
        {comments.map((comment) => {
          const isAuthor = comment.authorUid === userData.uid;
          const commentStyle = isAuthor
            ? {
                color: '#b388ff',
                textShadow: '0 0 6px #b388ff, 0 0 12px #b388ff',
                fontFamily: 'Orbitron, Segoe UI, Arial, sans-serif',
                marginBottom: '8px',
              }
            : {
                color: '#ff69b4',
                textShadow: '0 0 6px #ff69b4, 0 0 12px #ff69b4',
                fontFamily: 'Orbitron, Segoe UI, Arial, sans-serif',
                marginBottom: '8px',
              };
          return (
            ...
            >
              ...
              <small>
                By {comment.authorHandle} on {comment.createdOn}
              </small>
              <button
                onClick={() =>
                  setReplyTo(
              ...
              {replies
                ...
                .map((reply) => (
                  ...
                  >
                    ...
                    <small>
                      Reply by {reply.authorHandle} on {reply.createdOn}
                    </small>
                  </div>
                ))}
            </div>
